...
$cartera=$cn->query("SELECT * FROM cartera inner join acreedor on cartera.id_acreedor=acreedor.id WHERE id_acreedor = '$acreedor'");
 ...
 // echo $acreedor;
  $verdatos = "SELECT * FROM deudor inner join cartera_deudor_codeudor on deudor.id=cartera_deudor_codeudor.id_codeudor inner join cartera on cartera_deudor_codeudor.id_cartera=cartera.id where id_acreedor = '$acreedor' ";
// carga de vista
...
#alerta{
  ...
  <div class="container-fluid" id="alerta">
          <p class="text-white">DATOS DE LA CARTERA</p>

  
    <section class="mb-1">
    <?php if (mysqli_num_rows($verdatos) != 0) {
      $ver = mysqli_fetch_assoc($cartera);
     ?>
      ...
      <table class="table table-bordered table-hover table-condensed">
        <thead>
          <tr>
            ...
            <th class="text-white" style="background:#000080;">CODEUDOR</th>
            <th class="text-white" style="background:#000080;">DEUDA TOTAL</th>
            <th class="text-white" style="background:#000080;">OBSERVACIONES</th>
            <th class="text-white" style="background:#000080;">ACUERDO</th>
            <th class="text-white" style="background:#000080;">DOCUMENTOS</th>
          </tr>
        </thead>
        <tbody>
        <?php $i=1; while ($verdatos = mysqli_fetch_assoc($verdatos)) { ?>
          <tr>
            <td><?= $i++ ?></td>
            <td><?= $ficha['codigo'] ?></td>
            <td><?= $ficha['codigo'] ?></td>
